...
;(function ( $, window, undefined ) {
  ...
  var defaults = {
    ...
    defaultOffset: 0,
    lastDropRegion: undefined,
    hideResponseContainer: false
  };
  ...
  Pep.prototype.init = function () {
    ...
    //set initial position to all revert of all elements 
    //this.initialPosition = this.$el.position();//fix for a problem
    this.initialPosition = { 'left' : this.offset.left, 'top' : this.offset.top } 
  ...
  Pep.prototype.placeObject = function() {
    ...
    // it already has one.
    if ( parseInt( this.$el.css('left'), this.options.startingPosition[0] ) )
      this.offset.left = this.$el.css('left');

    if ( parseInt( this.$el.css('top'), this.options.startingPosition[1] ) )
      this.offset.top = this.$el.css('top');

    if ( this.options.removeMargins )
      this.$el.css({margin: 0});
      
    if ( this.options.startingPosition[0] )
    {        
        this.offset.left = this.options.startingPosition[0];
    }
    
    if ( this.options.startingPosition[1] )
    {        
        this.offset.top = this.options.startingPosition[1];
    }
  ...
  Pep.prototype.calculateActiveDropRegions = function() {
    ...
    $.each( $(this.options.droppable), function(idx, el){
      ...
      if ( self.isOverlapping($el, self.$el) ){
        if(!self.activeDropRegions.length)//there can bo only one
        {
  ...
  Pep.prototype.getMovementValues = function() {
   ...
   //Added by Robert The AWESOME
   Pep.prototype.getDefaultOffset = function() {        
        return this.options.defaultOffset;        
   };
   //THIS ONE IS ALSO AWESOME
   Pep.prototype.getHideResponseContainer = function() {        
        return this.options.hideResponseContainer;
   };
